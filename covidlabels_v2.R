covidlabels_v2=function(){
  ...
  P3=c(rep("Placebo",3),rep("Aspirin",3)) # cohort 2
  arm1=c(1,1,2,2,3,3) # for cohort 1
  arm2=c(1,1,1,2,2,2) # for cohort 2
  set.seed(228188)
  ...
  for ( i in 7 : 12){
    FF=write_data_frame(Allocation1,arm1,P2,(i-6),"Cohort 1 - Household Contacts")
    body_add_flextable(word_export, FF)
    body_add_break(word_export, pos = "after")
    # body_add_par(word_export, value = "")
    # body_add_par(word_export, value = "")
  ...
  FF=align(FF, i = NULL, j = c(3,4,5,6), align = "center", part = "all")
  FF= autofit(FF)
  return(FF)
}
