#!/usr/bin/env Rscript
run_HPV5_HPC=function(){
  args = commandArgs(trailingOnly=TRUE)
  pathin="~/HPV/"
  lower_age = as.integer(args[1])
  upper_age = as.integer(args[2])
  pathout=paste0("~/HPV/run",lower_age,"_",upper_age,"/")
  probs = rep(2:15,each = 70)
  prob_val =  probs[as.integer(args[3])]
  village_percent = 25
  compound_percent = 25
  house_percent = 50
  subject_percent = 50
  ...
  require(ICCbin)
  setDTthreads(1)
  total_sampling = matrix(data = 0, nrow = Nsim,ncol = 4)
  Pout=vector(mode="double",length=Nsim)
  pathname = getwd()
  ...
  for ( i in 1 : Nsim){
    ...
    Pout[i]=L$prob_estimate
    total_sampling[i,]=L$nums
  }
  saveRDS(list(Tot=total_sampling,P=Pout),paste0(pathout,"runout",args[3],".rds"))
}
