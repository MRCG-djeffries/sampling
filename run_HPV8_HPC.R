...
run_HPV8_HPC=function(){
  ...
  Nd=nrow(d)
  # these are now CVS
  vill_var=20#0.2#1 # written as percenatge
  comp_var=10#0.1#0.5
  hous_var=10#0.1#0.5
  ICC=0.1
  Nsim=20
  ...
  pathout=paste0("~/HPV/HPV5/run_",args[1])
...
simHPV=function(prob_val,village_percent,compound_percent,house_percent,subject_percent,vill_var,comp_var,hous_var,ICC,vcounts,compund_in_village,household_level,q){
  ...
  for ( i in 1 :length(vsamp)){
    village_p=max(0.00001,rnorm(1,mean=prob_val,sd=prob_val*vill_var))
    csamp = compund_in_village$Compound[compund_in_village$Village==vsamp[i]]
    for ( j in 1 :  length(csamp)){
      compound_p=max(0.00001,rnorm(1,mean=village_p,sd=village_p*comp_var))
      ...
      for (k in 1 : length(hsamp)){
        ...
        house_p=max(0.00001,rnorm(1,mean=compound_p,sd=compound_p*hous_var))
        if (length(m)==1){
